;; this stuff is just copied from here:
;; https://svn.red-bean.com/repos/kfogel/trunk/.emacs
;; because i don't know enough elisp to rewrite it myself
;; i barely know what this shit does lmao
...
;; let's see some introduction when we start up
(setq initial-scratch-message
      (concat ";; " (emacs-threat-of-the-day) "
")) ; i don't know whether an escape sequence would be parsed here

(defun kf/write-threat (&optional frame)
  "Write a threat into the scratch and minibuffer.
FRAME does nothing at all; it is included to work
with after-make-frame-functions."
  (interactive)
  (let ((scratch (get-buffer "*scratch*"))
	(threat (emacs-threat-of-the-day)))
    (save-current-buffer
      (set-buffer scratch)
      (goto-char (point-min))
...
(add-hook 'after-make-frame-functions 'kf/write-threat)
